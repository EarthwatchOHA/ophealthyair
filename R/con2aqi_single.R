con2aqi_single = function(pollutant, con, over_replace_value, type = NULL, na.ignore = FALSE) {
  il = c(0, 51, 101, 151, 201, 301, 401)
  ih = c(50, 100, 150, 200, 300, 400, 500)
  if(!na.ignore & is.na(con)){
    stop("NA value received, but na.rm = FALSE.")  
  } else if (na.ignore & is.na(con)) {
    return(NA)
  } else {
    if (pollutant == "o3") {
      if (type == "8h") {
