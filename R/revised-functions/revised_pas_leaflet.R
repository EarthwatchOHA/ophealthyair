revised_pas_leaflet <- function (pas = NULL, parameter = "pm25_1hr", paletteName = NULL, 
                                 radius = 10, opacity = 0.8, maptype = "terrain", outsideOnly = TRUE, group = NULL) 
{
  ...
  if (!(parameter %in% c("pm25_current", "pm25_10min", "pm25_30min", 
                         "pm25_1hr", "pm25_6hr", "pm25_1day", "pm25_1week", "humidity", 
                         "pressure", "temperature", "pwfsl_closestDistance"))) 
  ...
  suppressWarnings({
    if (stringr::str_detect(tolower(parameter), "^pm25")) {
      if (tolower(paletteName) == "aqi" || is.null(paletteName)) {
        ...
        colorInfo <- pas_palette(pas, paletteName = paletteName, 
                                 parameter = parameter, reverse = TRUE)
        cols <- colorInfo$colors
        colors <- colorInfo$key[, 2]
        labels <- c("0-12", "12-35", "35-55", "55-150", 
                    "150-250", ">250")
        legendTitle <- '\U00B5g/m3'
    ...
    else if (stringr::str_detect(parameter, "[dD]istance$")) {
      colorInfo <- pas_palette(pas, "distance", reverse = TRUE)
      cols <- colorInfo$colors
      labels <- colorInfo$key[, 1]
      colors <- colorInfo$key[, 2]
      ...
      colors <- c("#7f3b08", "#b35806", "#e08214", "#fdb863", 
                  "#fee0b6", "#d8daeb", "#b2abd2", "#8073ac", 
                  "#542788", "#2d004b")
      colorFunc <- leaflet::colorBin(colors, domain = range(pas[[parameter]]), 
                                     bins = 10, na.color = "#bbbbbb")
      cols <- colorFunc(pas[[parameter]])
      breaks <- seq(domain[1], domain[2], length.out = 11)
      offset <- diff(breaks)[1]/2
      levels <- signif(seq(breaks[1] + offset, breaks[11] - 
                             offset, length.out = 10), digits = 2)
      ...
      colorFunc <- leaflet::colorNumeric("Purples", domain = domain, 
                                         na.color = "#bbbbbb", reverse = FALSE)
      cols <- colorFunc(pas[[parameter]])
      breaks <- seq(domain[1], domain[2], length.out = 6)
      offset <- diff(breaks)[1]/2
      levels <- signif(seq(breaks[1] + offset, breaks[6] - 
                             offset, length.out = 5), digits = 4)
      colors <- (leaflet::colorBin("Purples", domain = range(breaks), 
    ...
    pas$popupText <- paste0("<b>", pas$label, "</b><br/>", 
  ...
  SPDF <- sp::SpatialPointsDataFrame(coords = cbind(pas$longitude, 
                                                    pas$latitude), data = as.data.frame(pas))
  ...
  m <- leaflet::leaflet(SPDF) %>% leaflet::setView(lng = mean(lonRange), 
                                                   lat = mean(latRange), zoom = zoom) %>% leaflet::addProviderTiles(providerTiles) %>% 
    leaflet::addCircleMarkers(radius = radius, fillColor = cols, 
                              fillOpacity = opacity, stroke = FALSE, popup = pas$popupText, 
                              layerId = pas$label, group = group) %>% leaflet::addLegend(position = "bottomright", 
                                                                          colors = rev(colors), labels = rev(labels), opacity = 1, 
                                                                          title = legendTitle)
  return(m)
}